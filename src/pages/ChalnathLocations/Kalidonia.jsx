...
export default function Kalidonia() {
  ...
  // ---------------- Derive Kalidonia planet + id (NO hardcoding) ----------------
  const kalidoniaPlanet = useMemo(
    () => planets.find((p) => p.name === "Kalidonia"),
    [planets]
  );
  const planetId = kalidoniaPlanet?.id ?? null;
  ...
  const savePlanet = async (id) => {
    setPlanetSaving(true);
    setPlanetError("");

    try {
      const res = await fetch(`${API_URL}api/campaign/planets/${id}`, {
        method: "PUT",
  ...
  const saveFaction = async (factionIdOrNew) => {
    ...
    if (!planetId) {
      setFactionSaving(false);
      setFactionError("Planet id not loaded yet.");
      return;
    }
    ...
    try {
      const res = await fetch(url, {
        ...
        body: JSON.stringify({
          campaign_id: 1,
          planet_id: planetId, // ✅ dynamic planet id (no hardcoding)
            ...
            .map((p) => {
              ...
              return (
                ...
                >
                  ...
                  >
                    <h2 style={{ color: "#4da6ff", margin: 0 }}>{p.name}</h2>
                  ...
                  <div style={{ marginTop: "0.9rem", display: "grid", gap: "0.8rem" }}>
                    {/* DETAILS */}
                    <div>
                      <strong style={{ color: "#a9d2ff" }}>Details</strong>
                      {!isEditing ? (
                        <p>{p.details}</p>
                      ) : (
                        <textarea
                          ...
                          onChange={(e) =>
                            setPlanetForm((prev) => ({ ...prev, details: e.target.value }))
                          }
                          style={planetInput}
                        />
                      )}
                    ...
                    {/* POPULATION */}
                    <div>
                      <strong style={{ color: "#a9d2ff" }}>Population</strong>
                      {!isEditing ? (
                        <p>{p.population || "—"}</p>
                      ) : (
                        <input
                          ...
                          onChange={(e) =>
                            setPlanetForm((prev) => ({ ...prev, population: e.target.value }))
                          }
                          placeholder="e.g. 6.8B"
                          style={planetInput}
                        />
                      )}
                    </div>
                    ...
                    {/* EXPORTS */}
                    <div>
                      <strong style={{ color: "#a9d2ff" }}>Exports</strong>
                      {!isEditing ? (
                        <p>{p.exports || "—"}</p>
                      ) : (
                        <input
                          ...
                          onChange={(e) =>
                            setPlanetForm((prev) => ({ ...prev, exports: e.target.value }))
                          }
                          placeholder="e.g. Grain, relics, mercenaries"
                          style={planetInput}
                        />
                      )}
                    </div>
                    ...
                    {/* ENVIRONMENT */}
                    <div>
                      <strong style={{ color: "#a9d2ff" }}>Environment</strong>
                      {!isEditing ? (
                        <p>{p.environment || "—"}</p>
                      ) : (
                        <input
                          ...
                          onChange={(e) =>
                            setPlanetForm((prev) => ({ ...prev, environment: e.target.value }))
                          }
                          placeholder="e.g. War-torn plains, fortress cities"
                          style={planetInput}
                        />
                      )}
                    </div>
      ...
      <section className="info-section">
        ...
        >
          ...
          {editingFactionId === "new" && (
            ...
            >
              <h3 style={{ color: "#4da6ff", marginTop: 0 }}>New Faction</h3>
          ...
          {factions.length > 0 ? (
            factions
              .filter((f) => planetId != null && f.planet_id === planetId)
              .map((f) => {
                ...
                return (
                  ...
                  >
                    <div style={{ display: "flex", justifyContent: "space-between", gap: "0.75rem" }}>
                      <h3 style={{ color: "#4da6ff", marginTop: 0 }}>
                    ...
                    {!isEditing ? (
                      <>
                        ...
                        {f.status && <p><strong>Status:</strong> {f.status}</p>}
                      </>
                    ) : (
                      <div style={{ display: "grid", gap: "0.6rem" }}>
                        <input
                        ...
                        <textarea
                          ...
                          value={factionForm.description}
                          onChange={(e) =>
                            setFactionForm((p) => ({ ...p, description: e.target.value }))
                          }
                          placeholder="Description"
                          style={planetInput}
                        />
                        <input
                          value={factionForm.territory}
                          onChange={(e) =>
                            setFactionForm((p) => ({ ...p, territory: e.target.value }))
                          }
                          placeholder="Territory"
                          style={planetInput}
                        />
                        <input
                          value={factionForm.exports}
                          onChange={(e) =>
                            setFactionForm((p) => ({ ...p, exports: e.target.value }))
                          }
                          placeholder="Exports"
                          style={planetInput}
                        />
                        <input
                          value={factionForm.status}
                          onChange={(e) =>
                            setFactionForm((p) => ({ ...p, status: e.target.value }))
                          }
                          placeholder="Status"
                          style={planetInput}
                        />
