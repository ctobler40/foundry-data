...
export default function Hrellik() {
  ...
  useEffect(() => {
    const fetchCharacter = async () => {
      try {
        const res = await fetch(`${API_URL}api/mainCharacters/1`); // Hrellik’s ID
        const data = await res.json();
        setCharacter(data);
      } catch (err) {
        console.error("Error loading Hrellik data:", err);
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) {
    return <div className="character-loading">Loading Hrellik’s profile...</div>;
  }
  ...
  return (
    <div className="profile-page">
      <div className="profile-header">
        ...
        <p className="profile-subtitle">The Lone Kroot of Nikonova</p>
      </div>
