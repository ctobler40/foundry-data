...
export default function Joe() {
  ...
  useEffect(() => {
    const fetchCharacter = async () => {
      try {
        const res = await fetch(`${API_URL}api/mainCharacters/4`); // Joe’s ID
        const data = await res.json();
        setCharacter(data);
      } catch (err) {
        console.error("Error loading Joe data:", err);
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) {
    return <div className="character-loading">Loading Joe’s profile...</div>;
  }
  ...
  return (
    <div className="profile-page">
      <div className="profile-header">
        ...
        <p className="profile-subtitle">The Reluctant Soldier</p>
      </div>
