...
export default function Dahlia() {
  ...
  useEffect(() => {
    const fetchCharacter = async () => {
      try {
        const res = await fetch(`${API_URL}api/mainCharacters/5`); // Dahlia’s ID
        const data = await res.json();
        setCharacter(data);
      } catch (err) {
        console.error("Error loading Dahlia data:", err);
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) {
    return <div className="character-loading">Loading Dahlia’s profile...</div>;
  }
  ...
  return (
    <div className="profile-page">
      <div className="profile-header">
        ...
        <p className="profile-subtitle">The Flame of the Faithless</p>
      </div>
