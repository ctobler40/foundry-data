...
export default function Kaleson() {
  ...
  useEffect(() => {
    const fetchCharacter = async () => {
      try {
        const res = await fetch(`${API_URL}api/mainCharacters/2`); // Kaleson’s ID
        const data = await res.json();
        setCharacter(data);
      } catch (err) {
        console.error("Error loading Kaleson data:", err);
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) {
    return <div className="character-loading">Loading Kaleson’s profile...</div>;
  }
  ...
  return (
    <div className="profile-page">
      <div className="profile-header">
        ...
        <p className="profile-subtitle">The Veteran Commander of the FAFO Unit</p>
      </div>
